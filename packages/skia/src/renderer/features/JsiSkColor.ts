import type { Color as InputColor, SkColor } from "../../skia/types/Color";
...
const parseCSSColor = (cssStr: string) => {
  ...
  const str = cssStr.replace(/ /g, "").toLowerCase();
  ...
  if (op !== -1 && ep + 1 === str.length) {
    ...
    switch (fname) {
      ...
      case "hsl": {
        ...
        const s = parseCSSFloat(params[1]);
        const l = parseCSSFloat(params[2]);
